...
public class ChessBoard extends JPanel {
        ...
        if (focusedPiece == null) {
            for (int i = 0; i < pawns.length; i++) {
                if (pawns[i].isPieceHit(x, y)) {
                    focusedPiece = pawns[i];
                    return focusedPiece;
                }
            }
            for (int i = 0; i < rooks.length; i++) {
                if (rooks[i].isPieceHit(x, y)) {
                    focusedPiece = rooks[i];
                    return focusedPiece;
                }
            }
            for (int i = 0; i < knights.length; i++) {
                if (knights[i].isPieceHit(x, y)) {
                    focusedPiece = knights[i];
                    return focusedPiece;
                }
            }
            for (int i = 0; i < bishops.length; i++) {
                if (bishops[i].isPieceHit(x, y)) {
                    focusedPiece = bishops[i];
                    return focusedPiece;
                }
            }
            for (int i = 0; i < queens.length; i++) {
                if (queens[i].isPieceHit(x, y)) {
                    focusedPiece = queens[i];
                    return focusedPiece;
                }
            }
            for (int i = 0; i < kings.length; i++) {
                if (kings[i].isPieceHit(x, y)) {
                    focusedPiece = kings[i];
                    return focusedPiece;
                }
            }
    ...
    public void paint(Graphics g) {
        ...
        if(isFirstLoad) {       // Prvotni nacteni
            ...
            // Pesaci
            for (int i = 0; i < pawns.length; i++) {
                paintPiece(g, pawns[i]);
            }
            // Veze
            for (int i = 0; i < rooks.length; i++) {
                paintPiece(g, rooks[i]);
            }
            // Kone
            for (int i = 0; i < knights.length; i++) {
                paintPiece(g, knights[i]);
            }
            // Strelci
            for (int i = 0; i < bishops.length; i++) {
                paintPiece(g, bishops[i]);
            }
            // Kralovny
            for (int i = 0; i < queens.length; i++) {
                paintPiece(g, queens[i]);
            }
            // Kralove
            for (int i = 0; i < kings.length; i++) {
                paintPiece(g, kings[i]);
            }
        } else if (lastWidth != this.getWidth() || lastHeight != this.getHeight()) {    // Aktualizace pozic pri zmene velikosti okna
            // Pesaci
            for (int i = 0; i < pawns.length; i++) {
            ...
            // Veze
            for (int i = 0; i < rooks.length; i++) {
                updatePiecesLocations(rooks[i]);
                paintPiece(g, rooks[i]);
            }
            // Kone
            for (int i = 0; i < knights.length; i++) {
                updatePiecesLocations(knights[i]);
                paintPiece(g, knights[i]);
            }
            // Strelci
            for (int i = 0; i < bishops.length; i++) {
                updatePiecesLocations(bishops[i]);
                paintPiece(g, bishops[i]);
            }
            // Kralovny
            for (int i = 0; i < queens.length; i++) {
                updatePiecesLocations(queens[i]);
                paintPiece(g, queens[i]);
            }
            // Kralove
            for (int i = 0; i < kings.length; i++) {
                updatePiecesLocations(kings[i]);
                paintPiece(g, kings[i]);
            }
